...
# ╔═╡ 836bf64a-6aec-11ef-1724-d72455872623
begin
	using LazyGrids
	using BlockArrays
	using Printf
...
# ╔═╡ 6bc53b9c-a00c-467e-a9ea-4d99173e2613
begin
	include("../dmsuite.jl")
...
# ╔═╡ db93d507-ed16-485a-a5d8-c0233f9d1c53
begin
	L = 8.0
	H = 1.0
	
	# y1, 𝒟ʸ = FourierDiff(800, 1);
	# y      = L/2π  * y1 |> collect
 #    𝒟ʸ     = (2π/L)^1 * 𝒟ʸ
	...
	y = range(-L/2.0, stop=L/2.0, length=1000) |> collect
	z = range(0.0,    stop=H,     length=400) |> collect
...
#####################
	...
	∂ᶻB₀₁   = gradient( B₀₁, z, dims=2 )
	
    ∂ʸU₀₁   = gradient( U₀₁, y, dims=1 )
    ∂ᶻU₀₁   = gradient( U₀₁, z, dims=2 )
	...
	@printf "min/max of Ro: %f %f \n" minimum(Ro) maximum(Ro)
	@printf "min/max of Ri: %f %f \n" minimum(Ri) maximum(Ri)
...
###########################
	# file = matopen("meanflow_beta2.2.mat", "w")
	# write(file, "y", y)
	# write(file, "z", z)
	# write(file, "B", B₀₁)
	# write(file, "U", U₀₁)
...
# ╔═╡ 5fc56017-103d-4fe2-86c3-99ef1907de1e
# let
# 	β₀ = range(0.5, 2.0, 10) |> collect
# 	Ro = zeros(10)
# 	for kt in 1:length(β₀)
# 		η₀ = zeros(length(y), length(z))
# 	    ξ₀ = zeros(length(y), length(z))
# 		for it in 1:length(y)
# 	        for jt in 1:length(z)
...
# 		Ro[kt]  = maximum(abs.(∂ʸU₀)) 
# 	end

# 	β  = range(0.5, 2.05, 100) |> collect

# 	fit = curve_fit(ExpFit, β₀, Ro)
	
# 	fig = Figure(fontsize=30, size = (800, 500), )

# 	ax1 = Axis(fig[1, 1], xlabel=L"$\beta$", xlabelsize=35,
#                         ylabel=L"$Ro_{max}$", ylabelsize=35, )
    
#  	scatter!(β₀, Ro, markersize=20, color=:black)
# 	lines!(β, β.^2/2, color=:red, linewidth=1)

# 	lines!(β, fit.(β), color=:black, linewidth=1)
...
# 	fig
# 	save("max_Ro_beta.png", fig, px_per_unit=8)
# end

# ╔═╡ 30868308-024a-4880-98f8-8fd0e90a14e9
let
	fig = Figure(fontsize=30, size = (1400, 500), )

	ax1 = Axis(fig[1, 1], xlabel=L"$y$", xlabelsize=35,
                        ylabel=L"$z$", ylabelsize=35, 
						title=L"$Ro \, (\beta=2.2)$")
    
    levels = range(-4, 4, length=10)
    co = contourf!(y, z, Ro, 
    ...
    ax2 = Axis(fig[1, 3], xlabel=L"$y$", xlabelsize=35,
                        ylabel=L"$z$", ylabelsize=35,
						title=L"$Ri \, (\beta=2.2)$")
    
    levels = range(0.25, 1.004, length=6)
    co = contourf!(y, z, Ri, 
		colormap=cgrad(:thermal, 	rev=false),
        levels=levels, extendlow = :auto, extendhigh = :auto )
	...
	#save("Ro_Ri_beta2.2.png", fig, px_per_unit=8)
	
	fig
end

# ╔═╡ c58a5d01-9429-4fce-af4d-1abbd7eeebca
let
	η = range(-6,   stop=6.0,  length=500) |> collect
	ζ = range(0.0,  stop=1.0,  length=60)  |> collect
	Σ, Ξ = ndgrid(η, ζ)
	...
	L₂ = 2.0/β₂
############################
	fig = Figure(fontsize=30, size = (1600, 600*1.7), )

	levelsᵦ  = LinRange(minimum(b₁), maximum(b₁), 10) #|> collect
	levels₋ = levelsᵦ[findall( x -> (x ≤ 0.0), levelsᵦ )]
	levels₊ = levelsᵦ[findall( x -> (x > 0.0), levelsᵦ )]
	@printf "B-contour interval: %f \n" levelsᵦ[2] - levelsᵦ[1]
	
	ax1 = Axis(fig[1, 1], ylabel=L"$z$", ylabelsize=50) #, aspect=1.8)
	
	levels = range(-0.25, 0.25, length=14)
	co = contourf!(y ./ L₁, z, U₀₁, colormap=cgrad(:RdBu, rev=true),
	    levels=levels, extendlow = :auto, extendhigh = :auto )
	
	# geostrophically adjusted buoyancy anamoly
	contour!(y ./ L₁, z, B₀₁, levels=levels₋, linestyle=:dash,  color=:black, 								linewidth=2.25) 
	contour!(y ./ L₁, z, B₀₁, levels=levels₊,  linestyle=:solid, color=:black, 								linewidth=2.25) 
	
	# initially imposed buoyancy anamoly
	contour!(η ./ L₁, ζ, b₁, levels=levels₋, linestyle=:dash,  color=:blue, 									linewidth=1.8) 
	contour!(η ./ L₁, ζ, b₁, levels=levels₊, linestyle=:solid, color=:blue, 									linewidth=1.8) 
	...
	#####
	ax2 = Axis(fig[1, 3], ylabel=L"$z$", ylabelsize=40) #, aspect=1.8)
	
	levels = range(-0.5, 0.5, length=14)
	co = contourf!(y ./ L₂, z, U₀₂, colormap=cgrad(:RdBu, rev=true),
	    levels=levels, extendlow = :auto, extendhigh = :auto )
	...
	tightlimits!(ax2)
	cbar = Colorbar(fig[1, 4], co, 
	        ticks=([-0.5, -0.25, 0, 0.25, 0.5], ["-0.5", "-0.25", "0", "0.25", "0.5"]))
	xlims!(-1.5, 1.5)
	ylims!(minimum(z), maximum(z))
	
	ax2.xticks=([-1.5, -0.75, 0, 0.75, 1.5], 
	            ["-1.5", "-0.75", "0", "0.75", "1.5"])
	
	ax1.title = L"$U (\beta=1)$"
	ax2.title = L"$U (\beta=2)$"
	...
	ax3 = Axis(fig[2, 1], xlabel=L"$y/(2/\beta)$", xlabelsize=40, 
	                    ylabel=L"$\partial_yB(z=1)$", ylabelsize=30, aspect=3)
	lines!(η ./ L₁,   ∂ηb₁[:,60], color=:blue, linewidth=3, label="Initial" )
	lines!(y ./ L₁,  ∂ʸB₀₁[:,length(z)], color=:black, linewidth=3, label="Adjusted")
	xlims!(-1.5, 1.5)
	ylims!(-0.6, 0)
	
	axislegend(; position=:lb, framevisible=false, patchsize=(35,20))
	
	ax3.yticks=([-0.6, -0.4, -0.2, 0], ["-0.6", "-0.4", "-0.2", "0"])
	
	ax4 = Axis(fig[2, 3], xlabel=L"$y/(2/\beta)$", xlabelsize=40, 
	                    ylabel=L"$\partial_yB(z=1)$", ylabelsize=30, aspect=3)
	lines!(η ./ L₂,   ∂ηb₂[:,60], color=:blue, linewidth=3, )
	lines!(y ./ L₂,  ∂ʸB₀₂[:,length(z)], color=:black,   linewidth=3, )
	xlims!(-1.5, 1.5)
	ylims!(-3.05, 0)

	ax1.xticks=([-1.5, -1, -0.5, 0, 0.5, 1, 1.5], 
	            ["-1.5", "-1", "0.5", "0", "0.5", "1", "1.5"])
	ax2.xticks=([-1.5, -1, -0.5, 0, 0.5, 1, 1.5], 
	            ["-1.5", "-1", "0.5", "0", "0.5", "1", "1.5"])
	ax3.xticks=([-1.5, -1, -0.5, 0, 0.5, 1, 1.5], 
	            ["-1.5", "-1", "0.5", "0", "0.5", "1", "1.5"])
	ax4.xticks=([-1.5, -1, -0.5, 0, 0.5, 1, 1.5], 
	            ["-1.5", "-1", "0.5", "0", "0.5", "1", "1.5"])
	
	hidespines!(ax3, :r, :t)
	hidespines!(ax4, :r, :t)
	
	Label(fig[1, 1, TopLeft()], L"$(a)$", fontsize=40, padding = (0, 0, 14, 0), 					halign = :right)
	Label(fig[1, 3, TopLeft()], L"$(b)$", fontsize=40, padding = (0, 0, 14, 0), 					halign = :right)
	Label(fig[2, 1, TopLeft()], L"$(c)$", fontsize=40, padding = (0, 0, 0,  0), 					halign = :right)
	Label(fig[2, 3, TopLeft()], L"$(d)$", fontsize=40, padding = (0, 0, 0,  0), 					halign = :right)

	fig
	save("mean_flow.png", fig, px_per_unit=4)
end
...
manifest_format = "2.0"
project_hash = "f4b884ba90f9e40e55b7ada4a44be5b7c10b9d3d"
...
[[deps.MatrixMarket]]
deps = ["CodecZlib", "LinearAlgebra", "SparseArrays"]
...
git-tree-sha1 = "496f3057920f5282e4a9166d415f513d20f1588b"
