...
# ╔═╡ 20a214c8-ab03-11ee-0015-5f3a0024992c
begin
	using LazyGrids
	using BlockArrays
	using Printf
...
# ╔═╡ fe0172c4-48eb-4fec-b53b-3d95ffc2b9af
begin 
	...
	max_val1 = maximum(real(λₛ₁))
	id = 0
	for it in 1:length(k₁)
	    if λₛ₁[it].re == max_val1
	        id = it
	    end
	end
	println(λₛ₁[id])
...
begin
	...
	#λₛ₁[len_-7] = 1.3λₛ₁[len_-7]
	λₛ₁[len_-6] = NaN #1.0λₛ₁[len_-6]
	λₛ₁[len_-5] = 1.5λₛ₁[len_-5]
	λₛ₁[len_-4] = 1.5λₛ₁[len_-4]
	λₛ₁[len_-3] = 1.5λₛ₁[len_-3]
	λₛ₁[len_-2] = 1.5λₛ₁[len_-2]

	#k₁[len_-5] = 1.03k₁[len_-5]
	...
	ax1.xticks=([0, 0.5, 1.0, 1.5], ["0", "0.5", "1", "1.5"])
	ax1.yticks=([0, 0.05, 0.1, 0.15, 0.2], 
				["0", "0.05", "0.1", "0.15", "0.2"])
	
	#ax2   = Axis(fig[1,2], xlabel=L"$k$", ylabel=L"$\sigma_r$")
	scatter!(k₂[1:27], real(λₛ₂[1:27]), markersize = 12, color = :blue) #, label="numerical solution")
	lines!(kₜₕ₂, λₜₕ₂, linewidth=3, color = :blue) #, label="Stone (1971)")
	
	text!(1.25, 0.15, text = L"δ=1",  color=:black, fontsize=40)
	text!(0.81, 0.09, text = L"δ=10", color=:blue,  fontsize=40)
	...
	ax2 = Axis(fig[1,2], xlabel=L"y", xlabelsize=45,
	        ylabel=L"z", ylabelsize=45, title=L"$\mathfrak{R}(\hat{w})$",
			titlesize = 40, 
	        xgridvisible = false, ygridvisible = false, 
	        aspect=1.1 )
	
	max_val = maximum(abs.(w_re))
	levels = range(0.0, max_val, length=8)
	co = contourf!(y1, z, w', colormap=cgrad(:Reds, rev=false),
	    levels=levels, extendhigh = :auto )

	cbar = Colorbar(fig[1, 3], co, 
		ticks=([0, 0.001, 0.002, 0.003], ["0", "10⁻³", "2×10⁻³", "3×10⁻³"]))
	
	xlims!(0., 1.0)
	ylims!(0., 1.0)

	ax2.xticks=([0, 0.5, 1.0], ["0", "0.5", "1"])
	ax2.yticks=([0, 0.5, 1], 
				["0", "0.5", "1"])
	...
	Label(fig[1, 1, TopLeft()], L"$(a)$", fontsize=40, 
							padding = (0, 0, 10, 0), halign = :right)
	Label(fig[1, 2, TopLeft()], L"$(b)$", fontsize=40, 
								padding = (0, 0, 10, 0), halign = :right)
...
# ╔═╡ adc9ea50-584a-4137-bf57-757795546e19
begin 
	@inline λₜₕ(k, Γ, δ) = 1.0/2√3 * ( k - 2.0k^3/15*(1.0 + 1.0/Γ + 5.0δ^2*k^2/42) )
	Γ = 0.5
	
	k = 0.2630612244897959      #0.89
...
manifest_format = "2.0"
project_hash = "c1c3e0aabc86d041855478543019eba0c53144bb"
...
git-tree-sha1 = "92a21f116834321b815ce74e0f3415191b58bf5b"
...
git-tree-sha1 = "382ecdebaf8364e11d4c2fe75e903884be0d85f4"
