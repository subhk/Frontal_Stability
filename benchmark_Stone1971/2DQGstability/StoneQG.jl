...
function BasicState!(diffMatrix, mf, grid, params)
    ...
    # y-gradient of basic-state PV
    ∂ʸQ₀    = zeros(size(Y))  

    @assert isapprox(∂ᶻU₀, -1.0∂ʸB₀; atol=1e-4)
...
@with_kw mutable struct Params{T<:Real} @deftype T
    ...
    Γ::T        = 0.01          # steepness of the initial buoyancy profile
    kₓ::T       = 0.0          # x-wavenumber
    E::T        = 1.0e-12      # Ekman number 
    Ny::Int64   = 32          # no. of y-grid points
    Nz::Int64   = 32           # no. of z-grid points
...
function EigSolver(Op, mf, params, emid, ra, x₀, σ, λ₀, it)
    ...
    if params.method == "feast"
        ...
        contour    = circular_contour_trapezoidal(emid, ra, 30)
    ...
    elseif params.method == "shift_invert"
        ...
        # λₛ = EigSolver_shift_invert_arpack_checking(𝓛, ℳ, σ₀=λₛ[1], α=0.15)
        # λₛ = EigSolver_shift_invert_arpack_checking(𝓛, ℳ, σ₀=λₛ[1], α=0.10)
        # λₛ = EigSolver_shift_invert_arpack_checking(𝓛, ℳ, σ₀=λₛ[1], α=0.06)
    ...
    elseif params.method == "arnoldi"
        ...
        # α = 0.25
        # λₛ = EigSolver_shift_invert_arnoldi_checking(𝓛, ℳ, σ₀=λₛ[1].re, α=α)
        ...
        pschur, residuals = jdqz(
                                𝓛, ℳ,
                                solver = JacobiDavidson.GMRES(size(𝓛, 1), iterations = 7),
                                target = target,
                                pairs  = 1,
...
function solve_Ou1984()
    ...
    #kₓ  = range(0.01, stop=80.0, length=800) |> collect  # β = 0.1
    #kₓ  = range(0.01, stop=60.0, length=800) |> collect  # β = 0.2
    kₓ  = range(0.01, stop=0.3, length=60) |> collect  # β = 0.2
    ...
    for it in 1:length(kₓ)
        ...
        if it == 1
            @time λₛ[it] = EigSolver(Op, mf, params, emid, 0.1ra, x₀, ra, λₛ, it)
            ...
            if params.method == "feast"; println("$emid ($ra)"); end
            if it == 2
                @time λₛ[it] = EigSolver(Op, mf, params, emid, 0.1ra, x₀, λₛ[it-1].re, λₛ, it)
                #@time λₛ[it], Χ = EigSolver(Op, mf, params, emid, ra, x₀, λ₂[it].re, it)
            else
                @time λₛ[it] = EigSolver(Op, mf, params, emid, 0.1ra, x₀, λₛ[it-1].re, λₛ, it)
                #@time λₛ[it], Χ = EigSolver(Op, mf, params, emid, ra, x₀, λ₂[it].re, it)
    ...
    Ny::Int = params.Ny
    Nz::Int = params.Nz 
    filename = "eigenvals_" * string(Nz) * string(Ny) * ".jld2"
